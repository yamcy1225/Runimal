...
public enum DefaultWorldContent {
    ...
    private static let fallbackPack = WorldContentPack(
        ...
        speciesBible: [
            species("windrunner", "에이라리스", "light", "먼 항로의 결을 깃결에 새기며 달린 이보다 먼저 바람길을 읽어내는 선도형 러니멀", "high", "high", "medium", "low", "low", "medium", ["urban", "riverside", "open-path"], ["wing", "trail", "feather"], ["가장 먼저 길의 숨을 듣는 종족", "멀어질수록 몸선이 오히려 더 또렷해지는 존재"], ["windrunner-gale-swift", "windrunner-loop-route", "windrunner-trail-wildland"]),
            species("stoneback", "크래그맨틀", "earth", "산마루의 중량과 인내를 갑각 아래 켜켜이 눌러 담는 방벽형 러니멀", "medium", "medium", "low", "high", "low", "low", ["ridge", "stairs", "rock"], ["shell", "ridge", "rock-heart"], ["등갑 하나에 언덕의 시간이 포개지는 종족", "느림을 밀도로 바꾸며 끝내 길을 지켜내는 존재"], ["stoneback-ridge-guard", "stoneback-summit-pulse", "stoneback-fault-wild"]),
            species("sparkfang", "신더래시", "flame", "질주의 열과 추격의 찰나를 송곳니 끝에 벼려 두는 화염형 러니멀", "low", "medium", "high", "low", "low", "medium", ["city", "sprint-lane", "signal-zone"], ["fang", "spark", "tail-flare"], ["찰나의 화염을 사냥하듯 좇는 종족", "리듬이 과열될수록 몸 가장자리에 잔열의 금이 도는 존재"], ["sparkfang-tempo-rhythm", "sparkfang-neon-urban", "sparkfang-burst-swift"]),
            species("mosshop", "모스베일", "leaf", "회복의 숨과 눅진한 정적을 몸 둘레에 드리워 주변 풍경까지 가라앉히는 치유형 러니멀", "medium", "high", "medium", "low", "low", "low", ["forest", "park", "rain-trail"], ["moss", "round", "soft-leaf"], ["쉼의 결을 잊지 않고 품는 종족", "숨을 고르게 할수록 주변 풍경까지 젖어 드는 존재"], ["mosshop-bloom-pulse", "mosshop-rain-wildland", "mosshop-canopy-guard"]),
            species("seedle", "던스프리그", "leaf", "탄생의 미약한 생기를 모아 새벽의 어린 순처럼 틔워 내는 발아형 러니멀", "medium", "medium", "medium", "medium", "low", "low", ["starter", "garden", "everywhere"], ["seed", "sprout", "tiny-tail"], ["아무 길에서나 첫 싹을 틔울 수 있는 종족", "가장 여린 모습으로 가장 많은 미래를 품는 존재"], ["seedle-sprout-swift", "seedle-root-guard", "seedle-twilight-bud"])
        ],
        mutationFamilies: [
